...
export default function EditCupon() {
  ...
  useEffect(() => {
    if (!id || !user?.is_staff) return
    async function load() {
      try {
        const res = await fetchWithAuth(`/coupons/${id}/`)
        if (!res.ok) throw new Error('Error cargando cupón')
        const c = await res.json()
        setCode(c.code || '')
        setDiscountType(c.discount_type || 'percent')
        setDiscountValue(String(c.discount_value || ''))
        setActive(Boolean(c.active))
        setValidFrom(new Date(c.valid_from).toISOString().slice(0, 16))
        setValidTo(new Date(c.valid_to).toISOString().slice(0, 16))
        ...
        <main className="flex-1 container mx-auto px-4 py-12 flex flex-col items-center">
          ...
          {!user?.is_staff ? (
            <div className="rounded-2xl border border-white/10 bg-white/5 backdrop-blur-xl p-6 max-w-2xl w-full text-gray-300">
              Solo administradores pueden editar cupones.
            </div>
          ) : (
